		...
		if (editAll) {
			try {
				...
				const deleteResult = await dispatch(
					removeScheduleToFirestore('jo97j5yp0CY0QDMHLaFuuothuBs2', scheduleIdsToDelete),
				);
				...
				const addResult = await dispatch(
					addScheduleToFirestore('jo97j5yp0CY0QDMHLaFuuothuBs2', updatedSchedules),
				);
			...
			try {
				// 기존 스케줄 삭제
				const deleteResult = await dispatch(
					removeScheduleToFirestore('jo97j5yp0CY0QDMHLaFuuothuBs2', [schedule.schedule_id]),
				);
				if (!deleteResult.success) {
				...
				if (hasRepeat) {
					...
					const editResult = await dispatch(
						editScheduleToFirestore('jo97j5yp0CY0QDMHLaFuuothuBs2', [updatedSchedule]),
					);
